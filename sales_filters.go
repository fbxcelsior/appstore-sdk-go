...
func (f *SalesReportsBaseFilter) ToQueryParamsMap() map[string]interface{} {
	...
	if !f.ReportDate.IsZero() {
		switch f.Frequency {
		case SalesReportFrequencyDaily:
		case SalesReportFrequencyWeekly:
			qs["filter[reportDate]"] = f.ReportDate.Format("2006-01-02")
			break
		case SalesReportFrequencyMonthly:
			qs["filter[reportDate]"] = fmt.Sprintf("%d-%02d", f.ReportDate.Year(), f.ReportDate.Month())
			break
		case SalesReportFrequencyYearly:
